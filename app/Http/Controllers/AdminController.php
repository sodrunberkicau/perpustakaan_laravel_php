...
class AdminController extends BaseController
{
    ...
    function addBook(Request $request, UserProfileProvider $UserProfileProvider)
    {
        ...
        // Simpan cover ke Object Storage (S3-compatible) agar URL COVER_PATH valid
        // Jika gagal (misalnya layanan S3 belum aktif), tidak melempar error di sini
        // supaya proses tambah buku tetap berjalan.
            $image->move(public_path('img/cover'), $imageName);
    ...
    function deleteBook(Request $request, UserProfileProvider $userProfileProvider)
    {
        if ($userProfileProvider->isAdmin()) {
            ...
            // Hapus cover dari S3 (jika ada). Jika gagal, diabaikan.
            if (file_exists(public_path('img/cover/' . $book->cover))) {
                unlink(public_path('img/cover/' . $book->cover));
            }
    ...
    function editBook(string $id, Request $request, UserProfileProvider $userProfileProvider)
    {
        ...
        if (($request->cover != null && $request->cover != '') && $book->cover != $request->cover) {
            ...
            if ($book->cover && file_exists(public_path('img/cover/' . $book->cover))) {
                unlink(public_path('img/cover/' . $book->cover));
            }
        }
